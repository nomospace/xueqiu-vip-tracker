    ...
    for vip in vips:
        ...
        for status in statuses:
            ...
            if status.created_at:
                try:
                    from datetime import datetime
                    status_created = datetime.fromisoformat(status.created_at.replace('Z', '+00:00').replace('+08:00', ''))
                except:
                    status_created = None
            ...
            saved = StatusAnalysis(
                ...
                status_created_at=status_created
            )
            db.add(saved)
            total_analyzed += 1
        await db.commit()
            ...
            if cached:
                ...
                try:
                    ...
                    if s.created_at:
                        try:
                            from datetime import datetime
                            status_created = datetime.fromisoformat(s.created_at.replace('Z', '+00:00').replace('+08:00', ''))
                        except:
                            status_created = None
...
async def fetch_all_timeline(
    ...
    # 检查内存缓存（10分钟有效，且数据不为空，且大部分有分析结果）
    if not data.force_refresh and cache_key in _daily_summary_cache:
        cached = _daily_summary_cache[cache_key]
        cached_data = cached["data"]
        
        # 检查缓存质量：数据不为空 且 大部分有分析结果
        total = cached_data.get("total", 0)
        statuses = cached_data.get("statuses", [])
        analyzed_count = sum(1 for s in statuses if s.get("analysis"))
        
        # 只有当数据不为空 且 分析率 > 80% 时才使用缓存
        if total > 0 and (analyzed_count / total) > 0.8 if total > 0 else False:
    ...
    try:
        ...
        for vip in vips:
            ...
            # 3. 评论回复 - 注意：雪球API的评论接口(type=4)返回的是历史数据(2020-2021年)，不是最近评论
            # 暂时禁用评论获取，避免返回无用的历史数据
            # try:
        ...
        # 确保所有动态都有 analysis 字段
        for status in unique_statuses:
            if "analysis" not in status:
                status["analysis"] = {
                    "coreViewpoint": "",
                    "relatedStocks": [],
                    "positionSignals": [],
...
class FetchTodayRequest(BaseModel):
    """拉取今日全部动态请求"""
    cookie: str = ""
    clear_today: bool = False  # 是否清理今日数据重新拉取


@router.post("/fetch-today-full")
async def fetch_today_full(
    data: FetchTodayRequest,
    db: AsyncSession = Depends(get_db)
):
    """
    拉取所有大V今日全部动态（增量拉取，遇到昨天的停止）
    
    特点：
    - 拉取全部今日动态，不限数量
    - 遇到昨天的数据自动停止
    - AI 分析后存入数据库
    - 支持 clear_today=true 清理今日数据重新拉取
    """
    import os
    import sys
    from datetime import datetime, timedelta
    ...
    try:
        ...
        if data.clear_today:
            ...
            await db.execute(
                delete(StatusAnalysis).where(StatusAnalysis.status_created_at >= today_start)
            )
            await db.commit()
            print(f"已清理今日 ({today}) 的分析数据")
        ...
        for vip in vips:
            stats["vips_processed"] += 1
            
            # 1. 拉取今日全部原发布动态 (type=0)
            try:
                timeline, reached = service.get_user_statuses_today(vip.xueqiu_id, status_type=0)
                if reached:
                    stats["reached_yesterday"] += 1
                ...
                print(f"[{vip.nickname}] 拉取到 {len(timeline)} 条原发布动态")
            except Exception as e:
                print(f"获取 {vip.nickname} 时间线失败: {e}")
            
            # 2. 拉取今日全部交易动态 (type=11)
            try:
                trades, reached = service.get_user_statuses_today(vip.xueqiu_id, status_type=11)
                if reached:
                    stats["reached_yesterday"] += 1
                ...
                print(f"[{vip.nickname}] 拉取到 {len(trades)} 条交易动态")
            except Exception as e:
                print(f"获取 {vip.nickname} 交易动态失败: {e}")
        ...
        stats["total_statuses"] = len(unique_statuses)
            ...
            if cached_analysis:
                ...
                try:
                    ...
                    if analysis.error:
                        ...
                        if status.get("created_at"):
                            try:
                                status_created_at = datetime.fromisoformat(
                                    status["created_at"].replace("Z", "+00:00").replace("+08:00", "")
                                )
                            except:
                                pass
        ...
        return {
            ...
            "stats": stats,
            "statuses": unique_statuses,
            "message": f"拉取完成：{stats['total_statuses']} 条动态，新分析 {new_analyzed} 条"
        }
...
async def check_analysis_health(db: AsyncSession = Depends(get_db)):
    """
    检查分析覆盖率健康状态
    ...
    result = await db.execute(
        select(StatusAnalysis)
        .where(StatusAnalysis.status_created_at >= week_ago)
    )
    all_analyses = result.scalars().all()
    
    total = len(all_analyses)
    has_analysis = sum(1 for a in all_analyses if a.core_viewpoint and a.overall_attitude)
    missing = total - has_analysis
    coverage_rate = (has_analysis / total * 100) if total > 0 else 0
    ...
    return {
        ...
        "health": health,
        "message": f"分析覆盖率 {coverage_rate:.1f}%，{'健康' if health == 'ok' else '需要关注'}"
    }


@router.post("/remedy-analysis")
async def remedy_missing_analysis(db: AsyncSession = Depends(get_db)):
    """
    自动补救缺失的分析
    
    遍历最近7天内缺少分析的动态，补充AI解读
    """
    import sys
    from datetime import datetime, timedelta
    from pathlib import Path
    ...
    try:
        ...
        return {"error": "未安装 stock-analyzer skill", "remedied": 0}
    ...
    service = XueqiuService()
    if not service.has_cookie():
        return {"error": "Cookie 未配置", "remedied": 0}
    ...
    for vip in vips:
        # 获取动态
        for status_type in [0, 11]:  # 原发 + 交易
            try:
                statuses = service.get_user_statuses(vip.xueqiu_id, status_type=status_type, count=30)
                ...
                for s in statuses:
                    ...
                    try:
                        status_time = datetime.fromisoformat(s.created_at.replace('Z', '+00:00').replace('+08:00', ''))
                        if status_time < week_ago:
                            continue
                    except:
                        continue
                    ...
                    # 分析
                    try:
                        analysis = analyze_text(s.text, s.title)
                        
                        if analysis.error:
                            errors.append({"status_id": s.id, "error": analysis.error})
                            continue
                        ...
                        saved = StatusAnalysis(
                            ...
                            summary=analysis.summary,
                            raw_content=s.text[:1000],
                            status_created_at=status_time
                        )
                        db.add(saved)
                        remedied += 1
                        
                    except Exception as e:
                        errors.append({"status_id": s.id, "error": str(e)})
                        
            except Exception as e:
                print(f"获取 {vip.nickname} 动态失败: {e}")
