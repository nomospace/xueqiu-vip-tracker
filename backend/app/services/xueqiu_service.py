...
class XueqiuService:
    ...
    def get_user_statuses_today(
        self,
        user_id: str,
        status_type: int = 0,
        max_pages: int = 20
    ) -> tuple[List[Status], bool]:
        """
        获取用户今日全部动态
        
        由于雪球数据不是按时间排序的，需要拉取多页后筛选今日动态。
        
        Returns:
            tuple: (今日动态列表, 是否已拉到昨天)
        """
        ...
        pages_without_today = 0  # 连续没有今日动态的页数
        
        for page in range(1, max_pages + 1):
            self._random_delay()
            
            # 注意：雪球 API count 参数最大支持 20
            statuses = self.get_user_statuses(user_id, status_type, count=20, page=page)
            ...
            if not page_has_today:
                pages_without_today += 1
            else:
                pages_without_today = 0  # 有今日动态就重置
            
            # 连续 3 页没有今日动态，停止
            if pages_without_today >= 3:
                print(f"[{user_id}] 连续 {pages_without_today} 页无今日数据，停止")
                break
            
            # 如果连续 2 页都少于 10 条数据，可能没有更多了
            if len(statuses) < 10 and pages_without_today >= 1:
                break
        
        return all_statuses, reached_yesterday
